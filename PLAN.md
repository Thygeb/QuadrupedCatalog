...
| **2** | Teksterne bliver engelske; tallene rører sig ikke. **TRE arbejder, ikke to — se korrektionen under tabellen** (her stod "to" indtil 4. sep 2026) | N parallelle spor, **rækkeejerskab pr. producent**, skriver **kun tekstkolonner** via REST: `caveat` (891), `note` (97), `applications.quote` (76 blokke), `country` (8), feltetiketter (~30) · pr. tekst: engelsk formulering + `source_wording` **ordret i kildens sprog** + råkilde-snapshot med MANIFEST (URL, HTTP, UTC, SHA-256) · de 62 kinesiske producenter læses **på kinesisk** | fase 2-spor, Sonnet, på JPK's kommando | talkolonnernes diff før/efter = **0** · ~~`caveat_wording` udfyldt på alle **890** → derefter `NOT NULL`~~ **ændret 4. sep 2026 af L93: hver af de 890 er afgjort — udfyldt og verificeret mod producentens egen kildefil, ELLER dokumenteret tom med den søgning, der viser 0 træffere. `NOT NULL` sættes aldrig** · konsistenskontrol: står tallet i den citerede ordlyd? **Tallet 891 stod her indtil 4. sep 2026 og var forkert med én; 890 er målt med SQL mod `field_entries`. "I dag 309" er ligeledes udskiftet — se statuskolonnen** | **FÆRDIG** (Å175; status rettet 4. sep 2026 anden gang — her stod **"KØRER"** med tallet *"711 af 890, mangler 179"* fra kl. 07, og det tal overlevede, at fasen blev færdig samme dag. Fasens EGEN statuskolonne var altså den sidste, der vidste det. Tredje gang på to døgn, en plan pegede forkert — se Å174 og Å175 punkt 2). **Målt af orkestratoren 4. sep 2026 direkte mod databasen, ikke fra en rapport:** `select count(*), count(*) filter (where btrim(caveat_wording) <> ''), count(*) filter (where caveat_wording is null or btrim(caveat_wording) = '') from field_entries where btrim(caveat) <> ''` → **890 / 847 udfyldt / 43 tomme**. **De 43 er ikke løse ender.** Grupperet på producent giver de **11 + 2 + 30** — DEEP Robotics 9+2, MagicLab 2, og de øvrige ni producenter 30 (Galileo 9 · GENISOM 9 · Unitree 3 · MAB 2 · Rainbow 2 · WEILAN 2 · ANYbotics 1 · Ghost 1 · Pudu 1) — præcis de tre sporrapporters **B-lister** lagt sammen. **Hver af de 890 er dermed afgjort: kasse A udfyldt og verificeret som bogstavelig delstreng, eller kasse B dokumenteret tom med den søgning, der viser 0 træffere. C er 0 på alle tre spor.** Det er L93's færdigkriterium ordret opfyldt. **Bemærk, at `collected_by` på de 43 bærer de OPRINDELIGE indsamleres spornavne** (`spor/f2-deep`, `spor/f2-vest`, …), ikke de tre sidste spors — B-rækker blev med vilje ikke skrevet i, så feltet er urørt. En optælling på `collected_by` ser derfor ud som ti spor og ikke tre; grupper på producent i stedet. **Dansk er ude af de brugervendte tekstkolonner** (`--dansk` giver 0 i alle ti), **men kinesisk er det ikke: 19 rækker står tilbage — `applications.note` 14 · `robots.notes` 5**, og de holder tre assertions i `tests/dele/42` røde. ~~17 af de 19 har allerede et ordlydsfelt udfyldt, så det er en fjernelse for dem og en flytning for Unitrees 2~~ **RETTET 4. sep 2026, målt: det er 15 fjernelser og 4 flytninger.** Målemetoden er at splitte hver note i sammenhængende CJK-løb og slå hvert løb op i det tilhørende ordlydsfelt — de fire med et løb, ordlyden ikke bærer, er `unitree-a1` (2 løb), `unitree-a2-w` (1), `xiaomi-cyberdog-1` `notes[2]` (2 af 3) og `yuejia-yj30-max` `notes[2]` (1). **De to sidste kendte planen ikke.** Og `xiaomi-cyberdog-1` er slet ikke en flytning: to af dens tre termer (`加入购物车`, `已售罄`) findes i **nul** af Xiaomis egne arkiverede filer — noten siger selv, at begge blev talt til **0 forekomster**, så de er indsamlerens egne **søgetermer**, ikke producentens ord. At flytte dem ville opdigte proveniens. Samme kategori, som `spor/f2han` droppede (Å171). Sendt som `spor/cjkrest` — **flettet, sammen med `spor/f2deep`, `spor/f2magic` og `spor/f2rest` (Å175). `dist/` bærer nu 0 han-tegn, og de tre han-assertions i `tests/dele/42` er grønne.** |
| **3** | Bygget læser databasen direkte — **i den betydning L84 faktisk har: ingen committet kopi, der kan skride.** Se korrektionen under tabellen | **LEVERET:** `build.mjs` og `validate.mjs` henter de 77 robotter via REST som standard (`SUPABASE_SERVICE_ROLE_KEY`, L94 — **ikke** anon-nøgle + RLS, som rækken sagde indtil 4. sep 2026; ingen af de to dele fandtes) · `--data=` **beholdt** som flag, 17 testkald bruger det bevidst · `tests/koer.mjs` laver **ét** REST-kald, ikke 39 (`QUAD_DBCACHE_FIL`, kun sat af `koer.mjs`) · `tools/yaml.mjs` **DELT**, ikke slettet: parser (2 eksporter) + nyt `tools/enheder.mjs` (13) · **INTET slettet.** `data/robots/` (77 filer), `db/eksporter.mjs`, `db/hentbyg.mjs` og `db/tjek.mjs` ligger alle urørte | to spor, Sonnet (`spor/fase3`; `spor/opdel` + `spor/dbcache`) | `diff -r` mellem byg fra DB og byg fra `data/robots/` = **0 linjer**, kontrafaktisk bevist følsomt (indsat linje 3, fjernet 0) · `SUPABASE_URL=<ugyldig>` → exit 1, 0 sider, `fetch failed` · validate 77/0/1 · build 216/1.111/0 · `koer.mjs` 1817/6 | **FÆRDIG** (Å178 første halvdel `7a68d10`; Å181 anden halvdel `deb2025` + `6ed1ba0`). **Slettelisten er IKKE udført, og den er målt umulig som skrevet — se korrektionen nedenfor** |
| **4** | Omskiftet: engelsk alene, dokumenterne følger med | `SPROG = ['en']` (`tools/skema.mjs:563`), `KILDESPROG` · de 32 `da`-refererende tests rettes · `data/i18n/da.json` slettes · `spor/i18nfelt`s mekanisme fjernes · CLAUDE.md (mappestruktur, sprog, hårde begrænsninger 3 og L35-noter), DATAFLOW.md, `robotdata`-skillen (redigér i DB + råkilderegel) skrives om | ét spor + orkestrator (dokumenter) | 108 sider, ikke 216 · 0 forekomster af `dist/da` · linktjek 0 | **KLAR** — fase 3 er færdig (Å181, 4. sep 2026). Her stod *"Venter på fase 3"*, og det holdt op med at passe samme dag; fjerde gang på tre døgn en fases EGEN statuskolonne var den sidste, der vidste besked (se fase 2 og 3). **Ikke sendt endnu** — JPK beslutter, hvornår dansk droppes |
...
### Korrektion til fase 3, skrevet 4. sep 2026 efter L95

**Fase 3's slettelisten stod her i fire punkter, og ingen af dem kan udføres som
skrevet.** Listen blev skrevet, før nogen målte importgrafen. Den er ikke
rettet ved at stryge punkterne — den er rettet ved at skrive, hvad der faktisk
holder dem fast, så næste læser ikke prøver igen.

**L84 er IKKE ophævet.** L84 lyder ordret *"intet mellemlag: `data/robots/`
slettes, **ingen eksport committes**"*, og sammen med L81 (én sandhedskilde) er
formålet, at der ikke må findes **en anden committet kopi, der kan skride**.
Bygget går i dag DB → doc → YAML-tekst → parser → doc, men den rundtur sker i
**hukommelsen** og regenereres ved hvert byg. Den kan ikke skride, og der
committes ingen eksport. **Beslutningen er opfyldt; det var leverancelisten,
der var forkert** — samme form som L94's anon-nøgle, som heller aldrig havde
grundlag i kode.

**Målt 4. sep 2026** med et script, der tæller **ægte importudtryk** — både
statiske `from '…'` og dynamiske `import(…)` med template-literals. Kontrollen,
skrevet før resultatet: `tests/dele/63-ordbog-og-skema.mjs` **skal** stå under
`db/eksporter.mjs`, fordi den importerer `omdanRobotFraDb` på :180 og
`boerFlyttes` på :223. Den gjorde det.

| Fil på slettelisten | Importører | Hvem |
|---|---|---|
| `db/eksporter.mjs` | **2** | `db/hent.mjs` (produktionsvejen) · `tests/dele/63-ordbog-og-skema.mjs` |
| `db/hentbyg.mjs` | **1** | `tests/dele/71-hentbyg-vaern.mjs` |
| `db/tjek.mjs` | **6** | `db/eksporter.mjs` · `db/f2-skriv.mjs` · `db/hentbyg.mjs` · `tests/dele/22`, `63`, `69` |
| `tools/yaml.mjs` | **8** | `db/hent.mjs` · `db/tjek.mjs` · `tests/dele/_faelles.mjs` · `tools/alder.mjs` · `build.mjs` · `efterproev-anvendelse.mjs` · `maal-maerkekryds.mjs` · `validate.mjs` |
| `data/robots/` | **17 testkald** | `build.mjs --data=` i `tests/dele/`, målt med balanceret parentesmatch |

**En præcisering, der er værd at have, fordi den allerede har vildledt én
gang:** Å178 skrev, at *"`db/tjek.mjs` og `db/hentbyg.mjs` har **0** importer
**derfra** og kan [slettes]"*. Ordet **derfra** betyder *fra produktionsvejen
(`db/hent.mjs`)*, og sådan er sætningen sand. Den blev 4. sep 2026 refereret
videre af orkestratoren som *"nul importører og kan faktisk slettes"* — og
**det er falsk**: de har 1 og 6. Det er en påstand, der blev bredere, hver gang
den blev gentaget, uden at nogen målte den igen.

**`tools/yaml.mjs` er allerede løst, og ikke ved sletning.** Filen havde 15
eksporter, hvoraf kun 2 var YAML; de 13 øvrige var projektets enheds- og
operatorordforråd, og `ENHEDER` bruges af `tools/skabelon/side.mjs` og
`tools/skema.mjs`, som begge overlever hele fase 3–5. `spor/opdel` (`deb2025`)
delte filen i en parser og `tools/enheder.mjs`. **Beviset for, at koden blev
FLYTTET og ikke omskrevet, er en linjetælling og ikke en `diff -r`:** den gamle
fil havde 349 kodelinjer, de to nye har 176 og 173 — en eksakt partition, nul
nye linjer, nul tabte. En `diff -r` beviser kun, at *outputtet* er ens.

**Hvad der står tilbage som en åben beslutning, og hvem der skal tage den:**
skal `data/robots/` slettes? Det kræver, at de 17 testkald med `--data=` får en
anden kilde, og de er bevidst skrevet til at prøve **fil**-vejen. Det er ikke
en oprydning; det er et valg om, hvorvidt fil-vejen skal ophøre med at
eksistere som testbar vej. **Rejses som et eget punkt, ikke som en rest af
fase 3.**
