# BRIEF — `spor/f2-deep`: DEEP Robotics' tekst fra dansk til engelsk

**Worktree:** `C:/Praktik/websites/udstilling-wt-deep` · **Gren:** `spor/f2-deep`
**Forgrenet fra main `996a7bf`** (3. sep 2026).
**Forventet pris:** ~340k tokens (9 robotter × ~38k, målt i forrige runde — det er et
**gæt**, ikke et krav; rapportér dit faktiske forbrug, hvis du kan se det).

Du er ét af tre parallelle fase 2-spor. De to andre har MagicLab+Pudu og
WEILAN+Youbaote. **I deler database.** Rører du en robot uden for DEEP Robotics,
er sporet forkert, og det bliver målt i `change_log`.
...
- `fund/OPSKRIFT-fase2.md` — **din primære opskrift.** DEEP Robotics' kilder er
  engelske.
- `fund/OPSKRIFT-fase2-cjk.md` — læs den også. Den bærer fælder, der rammer dig,
  selv om din kilde ikke er kinesisk.
...
Kør disse to før du ændrer noget, og skriv dine egne tal i rapporten.
**Afviger de fra mine, er det din måling der gælder — meld afvigelsen.**

```
node db/fase2-tjek.mjs --dansk --producent="DEEP Robotics"
node db/fase2-tjek.mjs --tal   --producent="DEEP Robotics"
```
...
|---|---|---|
| `caveat` | 77 | **75** |
| `applications.note` | 9 | **9** |
| `applications.note_wording` | 2 | **2** |
| `applications.quote` | 8 | **1** |
| `applications.quote_wording` | 1 | **1** |
| `images.note` | 6 | **6** |
| `robots.notes` | 16 | **16** |
| `images.alt` | 8 | 0 |
| `caveat_wording`, `robots.notes_wording` | 0 | 0 |
| **I alt** | | **110** |

**Talaftrykket, `--tal`, giver i dag:**
`a171780a9161f141c0cbd9a63830f82501b8073f4885eec76afbacff8c08c501`

Det aftryk er hele beviset for, at **intet tal rørte sig**. Kør `--tal` igen som
allersidste handling. Er aftrykket et andet, har du rørt en talkolonne, og
sporet skal stoppe og melde det — ikke rette videre.

---

## 3. Opgaven

**Oversæt DEEP Robotics' 110 danske celler til engelsk**, efter L87's tekstregel
i `BRIEF-FAELLES.md`: ordlydsfelterne bærer kildens ord ordret; vores egen prosa
må stå, men **kun når hver påstand kan efterprøves i en råkildefil** — kan den
ikke, **slettes den, den oversættes ikke**.

DEEP Robotics har **9 robotter** (robot-id 2190–2198, verificér selv listen).
Råkilderne ligger i `media/_kilder/raa-kina-deep-magic-2026-08-19/` — kopieret
ind i din worktree, fordi mappen er gitignoreret og ikke følger med grenen.
**Finder du ikke belæg dér, så hent friskt over HTTP og skriv den nye
hentedato** — gør ikke som et tidligere spor og lad et snapshot fra én dato bære
et citat indsamlet en anden.
...
venter på JPK's beslutning. Ingen af dem er DEEP Robotics — rør dem ikke.**

---

## 5. Rækkeejerskab og filejerskab

**Rækker, du må skrive:** kun rækker hvis `robot_id` tilhører **DEEP Robotics**.
Nul rækker uden for det. Det efterprøves i `change_log` af mig ved flettet.

**Filer, du ejer** (ingen anden må røre dem, og du må ikke røre andre):

- `fund/FUND-f2deep.md` — din rapport
- `fund/BRIEF-f2-deep.md` — dette brief
- `fund/f2deep-*` — dine egne måle- og snapshotfiler, frit navngivet med det præfiks

**Rør ikke:** `data/robots/`, `tools/`, `assets/`, `tests/`, `STATUS.md`,
`CLAUDE.md`, `DESIGN.md`, `db/*.mjs`. Ingen af dem er en del af opgaven.
`db/f2-skriv.mjs` **bruges**, men ændres ikke.
...
din rapport. De fire, `BRIEF-FAELLES.md` glemte, og som **du har arbejde i**:

`note_wording` (2 danske) · `quote` (1) · `quote_wording` (1) · `alt` (0 hos dig)
...
1. **Ingen dansk tilbage hos DEEP Robotics.**
   `node db/fase2-tjek.mjs --dansk --producent="DEEP Robotics"`
   Giver i dag **110 danske celler**. Færdig, når summen er **0** — eller når
   resten er dokumenterede L87-tvivlstilfælde, ét pr. linje i rapporten med
   begrundelse. **Tallet 0 er et krav; hvor mange tvivlstilfælde der bliver, er
   ukendt — mål det, gæt det ikke.**
2. **Intet tal har rørt sig.**
   `node db/fase2-tjek.mjs --tal --producent="DEEP Robotics"`
   Giver i dag `a171780a…c8c08c501`. Færdig, når aftrykket er **uændret**.
3. **Nul rækker uden for dine robotter.**
   Din egen forespørgsel mod `change_log` på `changed_by = 'spor/f2-deep'`,
   joinet mod `robots.manufacturer`. Færdig, når **alle** rækker er DEEP Robotics.
   Giver i dag **0 rækker** i alt, fordi sporet ikke har skrevet endnu — det er
   kontrollinjen: ser du et tal her før du skriver, måler du forkert.
4. **Hvert beholdt citat er efterprøvet mod en råkilde.** Rapportér **to tal**:
...
`fund/FUND-f2deep.md`. Højst 60 linjer plus `spor`-skillens to obligatoriske
...
påstår — tallene, robot-id'erne, kolonnelisten, kildemappens indhold — så er
