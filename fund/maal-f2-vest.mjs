/* Maaleredskab for spor/f2-vest. Koeres FOER og EFTER arbejdet.
   Kopi af fund/maal-f2.mjs, RETTET: originalen kaldte process.exit(1) EFTER et fetch
   (exit-127-faelden fra BRIEF-FAELLES.md) - her bruges process.exitCode i stedet, og
   fejlstien kaster en fejl frem for at afbryde direkte, saa event loop toemmes normalt.
   Brug:  node fund/maal-f2-vest.mjs <robot_id[,robot_id...]> */
import fs from 'node:fs';
for (const l of fs.readFileSync('.env', 'utf8').split(/\r?\n/)) {
  const m = l.match(/^([A-Z_]+)=(.*)$/); if (m) process.env[m[1]] = m[2].trim();
}
const U = process.env.SUPABASE_URL, K = process.env.SUPABASE_SERVICE_ROLE_KEY;
if (!U || !K) { console.error('mangler SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY i .env'); process.exit(1); } // FOER foerste fetch - ufarligt
...
if (!ids) { console.error('brug: node fund/maal-f2-vest.mjs <robot_id[,robot_id]>'); process.exit(1); } // FOER foerste fetch - ufarligt
