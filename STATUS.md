...
| **Å17** | **Fingeraftryksvagten mangler sin tabel i den levende database.** Å14 er implementeret og flettet (`spor/vagt2`, 25. aug 2026), men `db/skema.sql`s nye afsnit 7 er ikke kørt endnu — `synk_aftryk` svarer 404 (PGRST205). Indtil da falder `migrer --til-db` tilbage til den gamle YAML-sammenligning; adfærden er altså uændret, og flettet er ufarligt (efterprøvet: en fuld migrering kørte igennem med rundtur 77/77 og en tydelig besked om, hvad der mangler). **JPK skal køre afsnit 7 i Supabase' SQL-editor**, hvorefter orkestratoren måler de to scenarier, der beviser fixet: en Studio-redigering skal give exit 1 med rækketallene urørt, og en robot tilføjet i YAML skal køre igennem uden at nægte. Konfidens på Å14's kerne er **lav**, indtil de to målinger er taget |
