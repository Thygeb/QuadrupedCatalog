...
export default async function koer(ctx) {
  ...
  // Sammenligningssidens EGEN plade er en ANDEN skabelon (sammenligning.mjs)
  // og maa IKKE vaere roert - QUAD- skal stadig staa der.
  const samlHtml = fs.readFileSync(path.join(ud, 'da', 'sammenligning', 'index.html'), 'utf8');
  ok('48.7: sammenligningssidens plade er UROERT (baerer stadig QUAD-)',
    samlHtml.includes('QUAD-'), 'punkt 2 maa kun raekke katalogsiden');
