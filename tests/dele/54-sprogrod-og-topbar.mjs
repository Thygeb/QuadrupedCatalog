...
export default async function koer(ctx) {
  ...
  for (const [sprog, T] of [['da', daJson], ['en', enJson]]) {
    ...
    if (!html) continue;
    const erKatalog = html.includes(`>${T.katalog_titel}<`)
      && /<form class="styr" id="styr"/.test(html)
      && html.includes('id="alle"');
    ok(`54.1.${sprog}: dist/${sprog}/index.html ER katalogsiden (titel "${T.katalog_titel}", `
      + 'filterformular og resultatgitter #alle findes)', erKatalog);
  }
  // REVERT-BEVIS: en side UDEN katalogets markoerer bestaar ikke 54.1's tjek.
  const ikkeKatalog = '<html><body><h1>En helt anden side</h1></body></html>';
  ok('54.1.revert: en side uden katalog-markoererne fejler PRAECIS samme tjek',
    !(ikkeKatalog.includes(`>${daJson.katalog_titel}<`)
