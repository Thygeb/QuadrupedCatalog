...
export default async function koer(ctx) {
  const { rod, tmp, node, ok, lasRobotter, skema } = ctx;
  ...
  if (b.status !== 0) {
    throw new Error(
      `katalog/producent-fixture: byg af tests/eksempel-robotter fejlede (exit ${b.status}) - ` +
        (b.stderr || "").trim(),
    );
  }
  ...
  );
  {
    ...
    const katalogDaKort = fs.readFileSync(
      path.join(dist, "da", "index.html"),
      "utf8",
    );

    ok(
      "4a: et bygget katalogkort indeholder ingen kort-fod",
      !katalogDaKort.includes("kort-fod"),
    );

    ok(
      '4b: et bygget katalogkort indeholder intet EU/CE-maerke (klassen "eu eu--" er vaek)',
      !/class="eu eu--/.test(katalogDaKort) &&
        !katalogDaKort.includes("eu-svar"),
    );
    ...
    // det i dag.
    const producentMapper = fs
      .readdirSync(path.join(dist, "da", "producenter"), {
        withFileTypes: true,
      })
      .filter((f) => f.isDirectory())
      .map((f) => f.name);
    ok(
      "4c-forudsaetning: mindst én producentside er bygget til at maale L32 paa",
      producentMapper.length > 0,
      `fandt ${producentMapper.length} producentmapper`,
    );
    const producentDa = producentMapper.length
      ...
      : "";
    ok(
      "4c: EU-sektionen og CE-saetningen er fjernet fra producentsiderne jf. JPKs beslutning",
      !producentDa.includes('class="eu-fund-linje"') &&
        !producentDa.includes('id="eu-h"'),
    );
  }
  ...
  );
  {
    ...
    const fixtureProducenter = new Set(
      fixtureRobotter.map((rb) => rb.producent),
    );

    const prodIndeksDa = fs.readFileSync(
      path.join(dist, "da", "producenter", "index.html"),
      "utf8",
    );
    const prodIndeksEn = fs.readFileSync(
      path.join(dist, "en", "producenter", "index.html"),
      "utf8",
    );
    ...
    const figurTdAntal = (prodIndeksDa.match(/<td class="figur">/g) || [])
      .length;
    const navneTdAntal = (prodIndeksDa.match(/<td class="prod-navne">/g) || [])
      .length;
    const forventetRaekker = fixtureProducenter.size;
    ok(
      `K11: producentoversigten har ${forventetRaekker} datarækker med fire <td> hver, ` +
      ...
      trIAlt - 1 === forventetRaekker &&
        tdAntal === forventetRaekker * 4 &&
        figurTdAntal === forventetRaekker &&
        navneTdAntal === forventetRaekker &&
    ...
    for (const [land, t] of perLand) {
      ...
      if (
        !bedstLand ||
        producenter > bedstTal.producenter ||
        (producenter === bedstTal.producenter &&
          land.localeCompare(bedstLand, "da") < 0)
      ) {
        bedstLand = land;
        bedstTal = { producenter, modeller: t.modeller };
      }
    }
    ...
    const forventetDa =
      `${bedstTal.producenter} af ${totalProducenter} producenter er fra ${bedstLand} ` +
      `og står for ${bedstTal.modeller} af de ${totalModeller} modeller i kataloget.`;
    ok(
      `K12: fordelingssaetningen matcher en uafhaengig optaelling af proevedatasaettet ("${forventetDa}")`,
    ...
    const forventetEn =
      `${bedstTal.producenter} of ${totalProducenter} manufacturers are from ${bedstLandEn} ` +
      `and account for ${bedstTal.modeller} of the ${totalModeller} models in the catalogue.`;
    ok(
      `K12: samme fordelingssaetning er oversat paa /en/ ("${forventetEn}")`,
