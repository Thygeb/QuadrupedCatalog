/**
 * 43-skala-og-kurs.mjs — nyttelast- og prisskalaen (L65c) og den omregnede
...
export default async function koer(ctx) {
  ...
  ok('43.4: kildens URL er en rigtig adresse, man kan slaa efter i',
    /^https:\/\/\S+\.\S+/.test(kurser.kilde.url), kurser.kilde.url);
  ok('43.5: kildens dato er en ISO-dato',
    /^\d{4}-\d{2}-\d{2}$/.test(kurser.kilde.dato), kurser.kilde.dato);
  ok('43.6: basisvalutaen har selv en kurs i filen',
  ...
  ok('43.7: EUR staar som 1 (ECB\'s egen notation)', kurser.per_euro?.EUR === 1,
  ...
  for (const sprog of ['da', 'en']) {
    ...
    ok(`43.8.${sprog}: kursens kilde staar som et link paa siden`,
    ...
    for (const [valuta, tal] of Object.entries(kurser.per_euro)) {
      ...
      ok(`43.9.${sprog}.${valuta}: kursen staar med alle sine cifre`,
    ...
    ok(`43.10.${sprog}: kursens dato staar paa siden`,
    ...
    ok(`43.11.${sprog}: der ER omregnede priser paa siden`, maerker > 0,
    ...
    for (const k of prisKort) {
      ...
      if (erBasis) {
        iBasis++;
        ok(`43.12.${sprog}: en pris i ${kurser.basis} baerer INTET omregningsmaerke`,
    ...
    ok(`43.13.${sprog}: begge slags priser findes at maale paa`,
    ...
    for (const k of prisKort) {
      ...
      ok(`43.14.${sprog}: det omregnede kort baerer stadig producentens egen valuta`,
    ...
    for (const navn of ['nyttelast', 'pris']) {
      ...
      ok(`43.15.${sprog}.${navn}: skalaen findes`, i > 0);
      ...
      ok(`43.16.${sprog}.${navn}: skalaen staar hidden og taendes af scriptet`,
      ...
      ok(`43.17.${sprog}.${navn}: der er trin at filtrere paa uden JavaScript`,
      ...
      ok(`43.18.${sprog}.${navn}: de uoplyste har deres egen raekke`,
      ...
      ok(`43.19.${sprog}.${navn}: den uoplyste raekke staar UDEN FOR trinlisten`,
        uoplystI > trinSlut,
      ...
      ok(`43.22.${sprog}.${navn}: skalaen er et rigtigt range-felt med navn`,
    ...
    ok(`43.23.${sprog}: kortene baerer nyttelastens raa tal`, raaTal > 0,
    ...
    ok('43.24.' + sprog + ': de uoplyste har INGEN tal-attribut',
      raaTal < kortIalt,
  ...
  ok('43.25: katalog.mjs laeser kursen fra data, ikke fra en konstant',
    /kurser\.json/.test(skabelon),
    'en kurs i koden er et tal uden kilde');
  ok('43.26: prisen falder ikke tavst ud paa en ukendt valuta',
  ...
  ok('43.27: katalog.js baerer ingen kopi af traerskellisten',
    !/\[\s*5\s*,\s*10\s*,\s*20\s*,\s*50\s*\]/.test(js),
    'to lister, der skal vaere ens, driver fra hinanden ved fjerde rettelse');
  ok('43.28: katalog.js overlader skalafacetterne til skalaen, ikke til passer()',
    /erSkala\(f\)/.test(js),
