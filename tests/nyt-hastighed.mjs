#!/usr/bin/env node
/**
 * tests/nyt-hastighed.mjs — spor/hastighed (26. aug 2026): kanonisk
 * visningsenhed for hastighed vendes fra m/s til km/h.
 *
 *   node tests/nyt-hastighed.mjs
 *
 * Selvstaendig fil, IKKE en tilfoejelse til tests/koer.mjs — den fil deles af
 * et andet spor lige nu (spor/hastighed maa ikke roere den, jf. briefet).
 *
 ...
 *      IKKE regnes om — de er allerede maalet, og en omregning ville risikere
...
console.log('1. Kanonisk visningsenhed for hastighed er km/h');
...
console.log('\n2. En post der allerede staar i km/h roeres slet ikke');
{
...
console.log('\n3. m/s -> km/h regnes praecist om (unitree-a1s egen post: 3.3 m/s)');
{
...
console.log('\n4. Interval-poster (min/maks) regnes ogsaa om, praecist');
{
...
console.log('\n5. De fire navngivne km/t-robotter (D-fund i briefet): heltal, ingen omregning');
{
  const forventet = {
...
console.log('\n6. Tilstande og fri tekst roeres ikke (uaendret regel, ikke ny — men skal blive ved med at holde)');
{
...
console.log('\n7. Fuldt byg (isoleret --ud=, roerer ikke det rigtige dist/): alle talposter viser km/h, aldrig m/s');
{
  const tmpUd = path.join(rod, 'tests', '.tmp-hastighed-dist');
...
console.log('\n8. Kommentarblokken i skema.mjs modsiger ikke laengere koden');
{
